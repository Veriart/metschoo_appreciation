<?php
// index.php
// Dynamic Invitation Webpage - Formal Green Theme with QR entry
    ...
    <script id="tailwind-config">
      tailwind.config = {
        ...
        theme: {
          extend: {
            colors: {
              secondary: "#E2C12F", // Refined sage-olive — formal accent
              primary: "#0E1E0A",   // Deep forest
              cardBg: "#22401A",    // Medium green
            },
            fontFamily: {
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      #hero::before {
        ...
        background-image: url("img/wisuda/WHY_0142.JPG");
        background-size: cover; background-position: center;
        filter: blur(5px); transform: scale(1.0); z-index: 0;
      }
      #hero::after {
        content: "";
        position: absolute; inset: 0;
        background: linear-gradient(180deg, rgba(14,30,10,0.55) 0%, rgba(14,30,10,0.94) 100%);
        z-index: 1;
      }
      #hero > * { position: relative; z-index: 2; }
      
      .glass-card {
        background: rgba(210,219,198,0.04);
        backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
        border: 1px solid rgba(210,219,198,0.08);
      }
      .glass-card-gold {
        background: rgba(139, 158, 50, 0.08);
        backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
        border: 1px solid rgba(139, 158, 50, 0.28);
      ...
      #main-nav.scrolled {
        background: rgba(14,30,10,0.97);
        backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
        box-shadow: 0 4px 30px rgba(0,0,0,0.6);
        border-bottom: 1px solid rgba(226,193,47,0.22);
        padding-top: 1rem; padding-bottom: 1rem;
      }
      #mobile-menu { transition: all 0.35s cubic-bezier(0.4,0,0.2,1); max-height: 0; overflow: hidden; }
      #mobile-menu.open { max-height: 400px; }
      .timeline-line { position: absolute; left: 50%; top: 0; bottom: 0; width: 2px; background: rgba(226,193,47,0.3); transform: translateX(-50%); }
      @media (max-width: 767px) { .timeline-line { left: 20px; transform: none; } }
      .cat-card { transition: all 0.4s ease; }
      .cat-card:hover { background: #1C3412; transform: translateY(-4px); }
      .cat-card:hover p { color: #E2C12F; }
      ...
      .form-input:focus { outline: none; border-color: #E2C12F; box-shadow: 0 0 0 3px rgba(226,193,47,0.16); }
      .stat-divider { border-right: 1px solid rgba(226,193,47,0.16); }
      @media (max-width: 767px) { .stat-divider { border-right: none; border-bottom: 1px solid rgba(226,193,47,0.16); } }
      
      /* Formal Green text selection */
      ::selection { background: #E2C12F; color: #0E1E0A; }
    </style>
    <!-- html2canvas and qrcode.js CDNs -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>
  </head>
  <body class="bg-[#0E1E0A] text-[#D2DBC6] font-body overflow-x-hidden overflow-hidden selection:bg-secondary selection:text-primary">
    ...
    <div id="cover-overlay" class="fixed inset-0 z-[150] flex flex-col items-center justify-center bg-[#0E1E0A] text-center px-5 overflow-hidden">
      ...
      <div class="relative md:glass-card rounded-3xl py-10 z-10 max-w-5xl mx-auto flex flex-col items-center">
        ...
        <?php else: ?>
            <p class="text-[#D2DBC6]/50 text-xs tracking-wider mb-10 italic">Tamu Undangan</p>
        <?php endif; ?>

        <button id="btn-open-invitation" class="inline-flex items-center gap-2 bg-secondary text-primary font-bold px-8 py-4 rounded-full text-xs tracking-widest uppercase hover:scale-105 active:scale-95 transition-all shadow-[0_0_30px_rgba(226,193,47,0.4)] duration-300">
    ...
    <!-- Loading Screen -->
    <div class="hidden fixed inset-0 bg-[#0E1E0A] flex flex-col items-center justify-center" id="loading-screen">
    ...
    <div class="fixed bottom-6 right-4 sm:right-8 flex flex-col gap-3 z-[55]">
      <button class="w-11 h-11 rounded-full glass-card-gold flex items-center justify-center shadow-lg hover:scale-110 transition-transform bg-[#0E1E0A]/60 border border-secondary/30" id="music-toggle" title="Toggle music">
    ...
    <main class="w-full">
      ...
      <section class="w-full mx-auto px-5 sm:px-8 py-20 sm:py-32 flex flex-col items-center text-center reveal" id="hero">
        <div class="w-full mx-auto px-5 sm:px-8 py-22 flex flex-col items-center text-center reveal" style="transition-delay:200ms">
          ...
          “Setiap perjalanan memiliki kisah. Setiap perjuangan meninggalkan jejak. <br> Dan setiap pencapaian layak untuk dirayakan.”  
          </p>
          
          <!-- Event Meta -->
          <div class="flex flex-wrap justify-center gap-4 sm:gap-8 mb-6 text-[#D2DBC6]/80 text-sm font-semibold tracking-wide bg-[#D2DBC6]/5 border border-[#D2DBC6]/10 rounded-2xl p-4 sm:p-5 backdrop-blur-md">
            <div class="flex items-center gap-2">
            ...
            <div class="flex items-center gap-2">
              ...
              Ballroom Metropolitan
            </div>
          </div>
          
          <!-- Countdown -->
          <div class="flex justify-center gap-3 sm:gap-4 mt-4 mb-10" id="countdown">
            <div class="glass-card-gold px-4 sm:px-6 py-4 rounded-2xl text-center min-w-[75px] sm:min-w-[90px]">
              ...
              <div class="text-[9px] sm:text-[10px] uppercase tracking-widest text-[#D2DBC6]/50 mt-1">Days</div>
            </div>
            <div class="glass-card-gold px-4 sm:px-6 py-4 rounded-2xl text-center min-w-[75px] sm:min-w-[90px]">
              <div class="font-display text-2xl sm:text-3xl font-bold text-secondary" id="hours">00</div>
              <div class="text-[9px] sm:text-[10px] uppercase tracking-widest text-[#D2DBC6]/50 mt-1">Hours</div>
            </div>
            <div class="glass-card-gold px-4 sm:px-6 py-4 rounded-2xl text-center min-w-[75px] sm:min-w-[90px]">
              <div class="font-display text-2xl sm:text-3xl font-bold text-secondary" id="minutes">00</div>
              <div class="text-[9px] sm:text-[10px] uppercase tracking-widest text-[#D2DBC6]/50 mt-1">Mins</div>
            </div>
            <div class="glass-card-gold px-4 sm:px-6 py-4 rounded-2xl text-center min-w-[75px] sm:min-w-[90px]">
              <div class="font-display text-2xl sm:text-3xl font-bold text-secondary" id="seconds">00</div>
              <div class="text-[9px] sm:text-[10px] uppercase tracking-widest text-[#D2DBC6]/50 mt-1">Secs</div>
            </div>
          </div>
          ...
          <div class="flex flex-col sm:flex-row gap-4 justify-center w-full sm:w-auto">
            ...
            <a class="border-2 border-secondary/50 text-secondary px-8 py-4 rounded-full font-bold text-sm tracking-widest hover:bg-secondary/15 transition-all backdrop-blur-sm text-center" href="#rsvp">
              CONFIRM ATTENDANCE
            </a>
          </div>
        </div>
        ...
        <!-- Scroll hint -->
        <!-- <div class="absolute bottom-8 left-1/2 scroll-indicator flex flex-col items-center gap-1 text-[#D2DBC6]/30">
      ...
      <!-- ===== EVENT DETAILS ===== -->
      <section class="py-20 sm:py-24 bg-[#0E1E0A]">
        <div class="max-w-6xl mx-auto px-5 sm:px-8 lg:px-16">
          <div class="text-center mb-12 reveal">
            ...
            <h2 class="font-display text-3xl sm:text-4xl font-bold text-[#D2DBC6]">Event Details</h2>
          </div>
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div class="bg-[#22401A] p-8 sm:p-10 rounded-3xl reveal shadow-lg border border-secondary/25">
            ...
            <div class="bg-[#22401A] p-8 sm:p-10 rounded-3xl reveal shadow-lg border border-secondary/25" style="transition-delay:150ms">
              ...
              <h3 class="font-display font-bold text-xl mb-3 text-[#D2DBC6]">Venue</h3>
              <p class="text-[#D2DBC6]/70 leading-relaxed mb-5">Ballroom Metropolitan, Cileungsi, Bogor.</p>
              <a href="https://maps.app.goo.gl/oArj6ESR98SdY7YF9" target="_blank" class="inline-flex items-center gap-2 text-secondary font-bold text-sm hover:gap-3 transition-all">
                Open in Maps <span class="material-symbols-outlined text-base">open_in_new</span>
              </a>
            </div>
            <div class="bg-[#22401A] p-8 sm:p-10 rounded-3xl reveal shadow-lg border border-secondary/25 sm:col-span-2 lg:col-span-1" style="transition-delay:300ms">
              <div class="w-14 h-14 bg-secondary/15 rounded-2xl flex items-center justify-center mb-6">
      ...
      <!-- ===== TIMELINE ===== -->
      <section class="py-20 sm:py-28 bg-[#182E10] border-y border-secondary/15" id="schedule">
        <div class="max-w-4xl mx-auto px-5 sm:px-8 lg:px-16">
          <div class="text-center mb-16 reveal">
            ...
            <h2 class="font-display text-3xl sm:text-4xl md:text-5xl font-bold text-[#D2DBC6]">
              Event Schedule
            </h2>
      ...
      <!-- ===== GALLERY ===== -->
      <section class="py-20 sm:py-28 bg-[#0E1E0A]" id="gallery">
        <div class="max-w-6xl mx-auto px-5 sm:px-8 lg:px-16">
          <div class="text-center mb-12 reveal">
            <span class="text-secondary text-xs font-bold tracking-[0.2em] uppercase mb-3 block">Our Best Moments</span>
            <h2 class="font-display text-3xl sm:text-4xl md:text-5xl font-bold text-[#D2DBC6]">Achievement Gallery</h2>
          </div>
      ...
      <!-- ===== RSVP ===== -->
      <?php if($code != ''){?>
      <section class="py-20 sm:py-28 bg-[#182E10]" id="rsvp">
        <div class="max-w-3xl mx-auto px-5 sm:px-8">
          
          <div id="rsvp-container" class="bg-[#22401A] p-8 sm:p-12 md:p-16 rounded-3xl sm:rounded-[3rem] shadow-2xl reveal border border-secondary/30 relative overflow-hidden transition-all duration-500">
              ...
              <form class="space-y-5" id="rsvp-form">
                ...
                <div class="grid sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <?php else: ?>
                        <select name="classroom" class="form-input w-full bg-[#0E1E0A] border border-secondary/25 rounded-2xl px-4 py-3.5 text-[#D2DBC6] text-sm transition-all focus:border-secondary appearance-none">
            ...
            <div id="ticket-container" class="<?= !$hasRsvp || $rsvpStatus === 'Absent' ? 'hidden' : '' ?> flex flex-col items-center">
              ...
              <div id="ticket-card" class="relative w-full max-w-sm bg-gradient-to-b from-[#22401A] to-[#0E1E0A] border border-secondary/40 rounded-3xl overflow-hidden shadow-2xl p-6 flex flex-col items-center">
                <!-- Circular Ticket side cutouts -->
                <div class="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-8 bg-[#182E10] rounded-r-full border-r border-t border-b border-secondary/20 -ml-0.5"></div>
                <div class="absolute right-0 top-1/2 -translate-y-1/2 w-4 h-8 bg-[#182E10] rounded-l-full border-l border-t border-b border-secondary/20 -mr-0.5"></div>
                ...
                <div class="w-full py-4 text-center">
                  <p class="text-[#D2DBC6]/50 text-[9px] uppercase tracking-widest mb-1">Nama Siswa / Tamu</p>
                  ...
                  <div class="grid grid-cols-2 gap-4 mt-4 text-left px-2">
                    <div>
                      <p class="text-[#D2DBC6]/50 text-[9px] uppercase tracking-wider">Kelas</p>
                      <p class="text-[#D2DBC6] font-bold text-xs" id="ticket-class"><?= htmlspecialchars($isStudent ? $classroom : 'Tamu Undangan') ?></p>
                    </div>
                    <div>
                      <p class="text-[#D2DBC6]/50 text-[9px] uppercase tracking-wider">Pendamping</p>
                      <p class="text-[#D2DBC6] font-bold text-xs" id="ticket-companion">
              ...
              <div class="flex gap-4 mt-6 w-full justify-center">
                <button id="btn-edit-rsvp" class="bg-[#D2DBC6]/5 border border-[#D2DBC6]/10 hover:bg-[#D2DBC6]/10 text-[#D2DBC6] font-semibold text-xs tracking-widest uppercase px-5 py-3.5 rounded-xl transition-all">
                  Ubah RSVP
                </button>
            ...
            <div id="absent-container" class="<?= !$hasRsvp || $rsvpStatus === 'Attending' ? 'hidden' : '' ?> text-center flex flex-col items-center py-6">
              ...
              <h3 class="font-display text-2xl font-bold text-[#D2DBC6] mb-2">Terima Kasih Konfirmasinya</h3>
              <p class="text-[#D2DBC6]/60 text-sm max-w-md mb-6">Anda telah mengonfirmasi bahwa Anda berhalangan hadir. Jika rencana Anda berubah, Anda dapat memperbarui RSVP Anda.</p>
      ...
      <!-- ===== FOOTER ===== -->
      <footer class="bg-[#0A1408] text-[#D2DBC6]/40 py-16 px-5 sm:px-8 border-t border-secondary/20">
        <div class="max-w-4xl mx-auto flex flex-col items-center gap-6">
          ...
          <p class="text-[10px] text-center tracking-widest uppercase text-[#D2DBC6]/30">© 2026 SMK Pariwisata Metland School. All Rights Reserved.</p>
        </div>
      </footer>
    </main>
    ...
    <div class="fixed inset-0 bg-black/90 z-[200] hidden items-center justify-center p-5 backdrop-blur-md" id="success-modal">
      <div class="bg-[#22401A] border border-secondary/35 w-full max-w-sm p-10 rounded-3xl text-center shadow-2xl">
        <span class="material-symbols-outlined text-secondary text-6xl mb-4" style="font-variation-settings:'FILL' 1">check_circle</span>
        <h3 class="font-display font-bold text-2xl text-[#D2DBC6] mb-3">Thank You!</h3>
        <p class="text-[#D2DBC6]/70 text-sm mb-6 leading-relaxed">Your confirmation has been received successfully.</p>
